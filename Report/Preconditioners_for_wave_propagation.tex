...
(c) & (d)
\end{array}$
\caption{Non-mangetic non-conducting sphere of unit radius discretised using 14989 unstructured tetrahedral elements using $p=0,1,2,3$. Computed residual for BDDC, Jacobi (local), and Multigrid preconditioners. Figure shows that both BDDC and Multigrid converge rapidly, and that the local preconditioner does not result in convergence. ($a$) $p=0$, ($b$) $p=1$, ($c$) $p=2$, ($d$) $p=3$,}
\label{fig:sphere_preconditioner_iterations}
\end{figure}

Timings were performed using wall clock times for a single threaded implementation using a dedicated workstation with an Intel i7-9700K CPU at 3.60GHz frequency with 64GB DDR4 RAM.
...
(c) & (d)
\end{array}$
\caption{Non-mangetic non-conducting sphere of unit radius discretised using 14989 unstructured tetrahedral elements using $p=0,1,2,3$. Computation time for BDDC, Jacobi (local), and Multigrid preconditioners. Figure shows that both BDDC and Multigrid converge rapidly, and that the local preconditioner does not result in convergence. ($a$) $p=0$, ($b$) $p=1$, ($c$) $p=2$, ($d$) $p=3$,}
\label{fig:sphere_preconditioner_iterations}
\end{figure}
