...
def mem_profile(Solver='multigrid'):
    ...
    mem, times = memory_usage((run, (), {'Solver':Solver}), interval=0.1, retval=True)
...
def plot_mem_usage(timings, mem, add_breakdown=True):
    ...
    if add_breakdown is True:
        t0 = 0
        for dt, labels in zip([def_time, mesh_ref_time, precond_ref_time, solve_time, postproc_time, error_time],
                              ['Preprocessing', 'Refining mesh', 'Constructing preconditioner', 'Solving', 'Postprocessing', 'Computing errors']):
